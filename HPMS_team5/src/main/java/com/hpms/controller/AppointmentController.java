...
public class AppointmentController {
    ...
    @PostMapping("/edit/{id}")
    public String editAppointment(@PathVariable Integer id, 
    ...
    @GetMapping("/view/{id}")
    public String viewAppointment(@PathVariable Integer id, Model model) {
        User currentUser = getCurrentUser();
        Appointment appointment = appointmentService.getAppointmentById(id);
        ...
        if (!isAdmin() && 
            currentUser.getId() != appointment.getPatient().getId()) {
            return "redirect:/appointments?error=Unauthorized access";
        }
        
        model.addAttribute("appointment", appointment);
        model.addAttribute("medicalRecord", appointment.getMedicalRecord());
        model.addAttribute("canEditMedicalRecord", isAdmin());
